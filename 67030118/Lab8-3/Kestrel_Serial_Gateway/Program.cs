...
app.MapGet("/api/telemetry", (TelemetryStateStore state) => {
    ...
    // คำนวณระดับการแจ้งเตือนจากเปอร์เซ็นต์ปัจจุบัน
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            if (availablePorts.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        _logger.LogWarning(" ไม่พบพอร์ต {Target} ในระบบ! ระบบจะเลือกพอร์ตอื่นให้อัตโนมัติ...", targetPort);
                    }
                ...
                {
                    _logger.LogWarning(" ไม่สามารถเปิด {Port} ({Msg}) -> สลับเข้าโหมดจำลอง", selectedPort, ex.Message);
                }
            }
            else
            {
