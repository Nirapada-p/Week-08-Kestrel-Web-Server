...
1. **คลิปวิดีโอสาธิตการทำงาน (15 - 30 วินาที)**
  ...
  https://youtube.com/shorts/OH9AMcixw-c?si=S9Zc9BGknYMlgVLO
...
```
1.pollTelemetry() ฟังก์ชันหลัก ทำงานวนซ้ำทุก 150ms ด้วย setInterval() แต่ละรอบจะ fetch('/api/telemetry') ไปขอข้อมูลล่าสุดจาก Server แล้วแปลงเป็น JSON
```
```
2.updateLeftWidget(percentage) เอาเปอร์เซ็นต์ที่ได้มาแปลงเป็นเลข 2 หลัก แล้วสั่ง "เปิด/ปิดหลอด" ในตัวเลข 7-Segment ให้ตรงกับเลขนั้น
```
```
3.updateRightWidget(percentage) เอาเปอร์เซ็นต์มาคำนวณว่าควรมีหลอดไฟติดกี่ดวง (จาก 10 ดวง) แล้วไล่ปรับความสว่างของหลอดแต่ละดวงให้ติด/ดับตามนั้น
```
